...
VID_FORMATS = ['mov', 'avi', 'mp4', 'mpg', 'mpeg', 'mkv']
...
class LoadImagesAndVideos:
    def __init__(self, rgb_path, ir_path, batch=1, vid_stride=1):
        """Initialize dataloader for images and videos, supporting various input formats."""
        self.rgb_path = rgb_path
        self.ir_path = ir_path
        self.batch = batch
        self.vid_stride = vid_stride
        self.files = self.load_files(rgb_path, ir_path)
        self.nf = len(self.files)
        self.video_flag = [False] * self.nf  # Assume all are images for simplicity
        self.mode = "image"  # Assume image mode for simplicity

        if self.nf == 0:
            raise FileNotFoundError(f"No images or videos found in {rgb_path} or {ir_path}.")

    def load_files(self, rgb_path, ir_path):
        """Load and match RGB and IR files."""
        rgb_files = sorted([os.path.join(rgb_path, f) for f in os.listdir(rgb_path) if os.path.isfile(os.path.join(rgb_path, f))])
        ir_files = sorted([os.path.join(ir_path, f) for f in os.listdir(ir_path) if os.path.isfile(os.path.join(ir_path, f))])
        
        if len(rgb_files) != len(ir_files):
            raise ValueError("The number of RGB and IR images must be the same.")
    ...
    def __next__(self):
        paths, rgb_imgs, ir_imgs, info = [], [], [], []
        while len(rgb_imgs) < self.batch:
            if self.count >= self.nf:  # end of file list
                if rgb_imgs:
                    return paths, rgb_imgs, ir_imgs, info  # return last partial batch
                else:
                    raise StopIteration

            rgb_path, ir_path = self.files[self.count]
            rgb_img = cv2.imread(rgb_path)
            ir_img = cv2.imread(ir_path, cv2.IMREAD_GRAYSCALE)

            if rgb_img is None or ir_img is None:
                raise FileNotFoundError(f"Error reading image {rgb_path} or {ir_path}")

            paths.append((rgb_path, ir_path))
            rgb_imgs.append(rgb_img)
            ir_imgs.append(ir_img)
            info.append(f"image {self.count + 1}/{self.nf} {rgb_path}, {ir_path}: ")

            self.count += 1  # move to the next file

        return paths, rgb_imgs, ir_imgs, info

    def __len__(self):
        return math.ceil(self.nf / self.batch)  # number of batches
